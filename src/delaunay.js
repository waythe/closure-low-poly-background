...
wythe.delaunay.circumcircle_ = function(vertices, i, j, k) {
    var x1 = vertices[i][0],
        y1 = vertices[i][1],
        x2 = vertices[j][0],
        y2 = vertices[j][1],
    ...
    if (fabsy1y2 < wythe.delaunay.EPSILON && fabsy2y3 < wythe.delaunay.EPSILON)
        throw new Error("Eek! Coincident points!");

    if (fabsy1y2 < wythe.delaunay.EPSILON) {
        m2 = -((x3 - x2) / (y3 - y2));
        mx2 = (x2 + x3) / 2.0;
        my2 = (y2 + y3) / 2.0;
        xc = (x2 + x1) / 2.0;
        yc = m2 * (xc - mx2) + my2;
    } else if (fabsy2y3 < wythe.delaunay.EPSILON) {
        m1 = -((x2 - x1) / (y2 - y1));
        mx1 = (x1 + x2) / 2.0;
        my1 = (y1 + y2) / 2.0;
        xc = (x3 + x2) / 2.0;
...
wythe.delaunay.dedup_ = function(edges) {
    var i, j, a, b, m, n;
...
wythe.delaunay.triangulate = function(vertices, key) {
    ...
    for (i = indices.length; i--; edges.length = 0) {
        ...
        for (j = open.length; j--;) {
            ...
            if (dx * dx + dy * dy - open[j].r > wythe.delaunay.EPSILON)
                continue;
...
wythe.delaunay.contains = function(tri, p) {
